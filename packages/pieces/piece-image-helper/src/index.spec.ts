...
describe('image helper execution', () => {
  it('downloads and writes an image', async () => {
    ...
    const create = vi.fn().mockResolvedValue({ url: '/media/image.gif' });
    const tool = imageHelper.rotateImage;
    try {
      await expect(
        tool.execute({ image: `http://127.0.0.1:${address.port}/image.gif`, degree: 90 }, {
          frogbot: { config: { files: { slug: 'media' } }, create },
        } as never),
      ).resolves.toBe('/media/image.gif');
    } finally {
