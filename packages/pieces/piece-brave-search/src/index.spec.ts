...
import { braveSearchActions, createBraveSearch } from './index.js';
...
it('sends the Brave credential through the real action', async () => {
  ...
  await tool.execute({ query: 'frogbot' }, {
    req: {},
    frogbot: {
      connections: { resolve: () => adaptCredential('secret_text', { apiKey: 'brave-test' }) },
    },
  } as never);
  expect(new Headers(fetch.mock.calls[0]?.[1]?.headers).get('X-Subscription-Token')).toBe(
    'brave-test',
  );
});
