...
export function SheetContent({
  ...
  return (
    <Primitive.Portal>
      <Primitive.Overlay className="fixed inset-0 z-50 bg-black/80" />
      <Primitive.Content
        className={cn('fixed z-50 bg-background p-6 shadow-lg', sides[side], className)}
        {...props}
      >
        {children}
        <Primitive.Close className="absolute right-4 top-4 opacity-70">
          <X className="size-4" />
