...
describe('toModelMessages', () => {
    ...
    expect(messages).toEqual([
      {
        ...
        content: [
          { type: 'text', text: 'describe this' },
          {
            type: 'file',
            mediaType: 'image/*',
            data: { type: 'url', url: new URL('https://example.com/frog.png') },
          },
          {
            type: 'file',
  ...
  it('maps developer array content to a system message', () => {
    const messages = toModelMessages([{ role: 'developer', content: 'be terse' }]);
  ...
  it('rejects provider file references instead of inventing file lookup semantics', () => {
    expect(() =>
      toModelMessages([{ role: 'user', content: [{ type: 'input_file', file_id: 'file_123' }] }]),
    ).toThrow(UnsupportedModalityError);
  });
