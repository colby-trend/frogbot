...
const tokenArraySchema = z
  .array(tokenSchema)
  .min(1, 'input token array must contain at least one token');
...
const embeddingsRequestSchema = z
  .object({
    ...
    input: z.union([
      ...
      tokenArraySchema,
      z
        .array(z.string().min(1, 'input strings must be non-empty'))
        .min(1, 'input array must contain at least one string')
        .max(2048, 'input array must contain at most 2048 items'),
      z
        .array(tokenArraySchema)
        .min(1, 'input array must contain at least one token array')
        .max(2048, 'input array must contain at most 2048 items'),
    ]),
