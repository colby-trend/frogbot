...
vi.mock('@payloadcms/next/withPayload', () => ({
  withPayload: <T>(config: T): T => config,
}));

const { withFrogbot } = await import('./withFrogbot.js');
...
describe('withFrogbot', () => {
  ...
  it('calls the consumer webpack config and appends native externals', () => {
    ...
    const result = config.webpack?.({ externals: ['base-external'] }, {} as never);

    expect(webpack).toHaveBeenCalledWith({ externals: ['base-external'] }, {});
    expect(result?.externals).toEqual(['consumer-external', '@basetenlabs/performance-client']);
  });
});
