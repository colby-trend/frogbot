...
function mergeCollection(user: CollectionConfig, base: CollectionConfig): CollectionConfig {
  for (const field of user.fields) {
    ...
    if (name && RESERVED_FIELDS.includes(name)) {
      throw new Error(
        `[frogbot] Field '${name}' on collection '${user.slug}' is reserved by connections.`,
      );
    }
  }
  ...
  if (metadata && (!('type' in metadata) || metadata.type !== 'json')) {
    throw new Error(
      `[frogbot] Field 'metadata' on collection '${user.slug}' must use type 'json'.`,
    );
  }
  return {
    ...base,
    ...
    fields: [
      ...user.fields,
      ...base.fields.filter(
        (field) => !user.fields.some((item) => fieldName(item) === fieldName(field)),
      ),
    ],
...
export function resolveConnectionsCollections(
  ...
  if (marked.length > 1) {
    throw new Error(
      `[frogbot] Multiple collections marked \`connections: true\` (${marked.map((c) => c.slug).join(', ')}). Mark exactly one.`,
    );
  }
  const enabled =
    pieces.pieces.some((piece) => piece.credentialType !== 'none') ||
    marked.length === 1 ||
    !!config.credentialSources?.length;
  const encryption =
  ...
  if (config.collections.some((collection) => collection.slug === slug)) {
    throw new Error(
      `[frogbot] Collection slug '${slug}' conflicts with the default connections collection. Add \`connections: true\` to adopt it, or rename it.`,
    );
  }
  return {
