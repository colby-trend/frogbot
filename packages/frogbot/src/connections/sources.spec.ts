...
describe('resolveCredentialSources', () => {
  it('selects a single claiming source', () => {
    expect(
      resolveCredentialSources({ sources: [source], assignments: {}, pieces: [piece] }),
    ).toEqual({ linear: 'one' });
  });
  ...
  it('fails for ambiguous, missing, and invalid sources', () => {
    const two = { ...source, key: 'two' } as const;
    expect(() =>
      resolveCredentialSources({ sources: [source, two], assignments: {}, pieces: [piece] }),
    ).toThrow('multiple credential sources');
    expect(() =>
      resolveCredentialSources({ sources: [], assignments: {}, pieces: [piece] }),
    ).toThrow('no source claims it');
    expect(() =>
      resolveCredentialSources({
        sources: [source],
        assignments: { linear: 'missing' },
        pieces: [piece],
      }),
    ).toThrow("cannot provide 'linear'");
  });
});
