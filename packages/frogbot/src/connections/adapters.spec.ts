...
describe('credential adapters', () => {
  ...
  it('adapts standard credential formats', async () => {
    await expect(adaptCredential('secret_text', { apiKey: 'key' })).resolves.toEqual({
      type: 'SECRET_TEXT',
      secret_text: 'key',
    });
    await expect(
      adaptCredential('basic_auth', { username: 'frog', password: 'bot' }),
    ).resolves.toEqual({ username: 'frog', password: 'bot' });
    await expect(adaptCredential('oauth2', { access_token: 'token' })).resolves.toEqual({
      type: 'OAUTH2',
      access_token: 'token',
    });
  });
  ...
  it('mints and caches impersonated service-account tokens', async () => {
    ...
    await expect(adaptCredential('service_account', credential)).resolves.toEqual({
      type: 'OAUTH2',
      access_token: 'token',
    });
    ...
    expect(JSON.parse(Buffer.from(assertion.split('.')[1]!, 'base64url').toString())).toMatchObject(
      { sub: 'user@example.com', scope: 'read' },
    );
  });
});
