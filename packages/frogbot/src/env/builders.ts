...
export type EnvBuilderOutput<TBuilder> = TBuilder extends EnvBuilder<infer T, infer TResolved>
...
const createBuilder = <T>(state: BuilderState<T>): EnvBuilder<T> => {
  const builder: EnvBuilderDescriptor<T> = {
    ...state,
    default(value) {
      if (state.requiredMode) throw new Error('An env variable cannot be both required and defaulted')
      return createBuilder({ ...state, defaultValue: value, hasDefault: true }) as EnvBuilder<T, true>
    },
    name(name) {
    ...
    required() {
      if (state.hasDefault) throw new Error('An env variable cannot be both required and defaulted')
      if (state.requiredMode) throw new Error('An env variable can only have one required modifier')
