...
export function mcpPlugin(pluginOptions: McpPluginOptions = {}): Plugin {
  ...
  return async (config) => {
    const apiKeyStrategy = config.collections.flatMap((collection) => {
    ...
    const { experimental: _experimental, overrideAuth: _overrideAuth, ...options } = pluginOptions as MCPPluginConfig;
    return await payloadMcpPlugin({
      ...options,
      overrideAuth: (req) => resolveMcpAccess({ authenticate: apiKeyStrategy.authenticate, pluginOptions: options, req }),
