...
export function apiKeysPlugin(options: ApiKeysPluginOptions = {}): Plugin {
  ...
  return (config) => {
    ...
    const reservedPolicyFields = new Set([
      'monthlyBudget',
      'rpm',
      'tpm',
      'models',
        ...
        for (const collection of [collectionSlug, authCollection]) {
          await req.payload.update({
            ...
            data: {
              ...
              budgetPeriodStartedAt: new Date().toISOString(),
              budgetAlertsSent: [],
            },
            overrideAccess: true,
            req,
    ...
    const jobs = {
      ...
      autoRun:
        typeof autoRun === 'function'
          ? async (payload: Parameters<typeof autoRun>[0]) => [
              ...(await autoRun(payload)),
              { allQueues: true, cron: '* * * * *' as const },
            ]
              ...
              hooks: {
                ...
                beforeOperation: [
                  ...
                  async (args) => {
                    ...
                    const key =
                      apiKeyId === undefined
                        ? undefined
                        : ((await args.req.frogbot.findByID({
                    ...
                    const policy = resolvePolicy({
                      key: key as PolicyDocument | undefined,
                      user,
                    ...
                    const subjects = [
                      ...
                      (
                        value,
                      ): value is { subject: string; policy: ReturnType<typeof resolvePolicy> } =>
                        Boolean(value),
                    );
                    if (
                      subjects.some(
                        ({ policy: current }) =>
                          current.budgetBehavior === 'block' &&
                ...
                afterOperation: [
                  ...
                  async (args) => {
                    ...
                    if (cost <= 0) return;
                    const targets = [
                      req.user.apiKeyId === undefined
                        ? undefined
                        : { collection: collectionSlug, id: req.user.apiKeyId },
                      { collection: authCollection, id: req.user.id },
                    ].filter((target): target is { collection: string; id: string | number } =>
                      Boolean(target),
                    );
                    await Promise.all(
                      targets.map((target) =>
                        queue.run(`${target.collection}:${target.id}`, async () => {
                          ...
                          const spend =
                            (typeof doc.spendThisPeriodUSD === 'number'
                              ? doc.spendThisPeriodUSD
                              : 0) + cost;
                          const sent = Array.isArray(doc.budgetAlertsSent)
                            ? doc.budgetAlertsSent.filter(
                                (value): value is number => typeof value === 'number',
                              )
                            : [];
                          const policy = resolvePolicy({
                            key: (target.collection === collectionSlug
                              ? doc
                              : context.policyKey) as PolicyDocument | undefined,
                            user: (target.collection === authCollection
                              ? doc
                              : req.user) as PolicyDocument,
                            defaults: options.defaults,
                          });
                          ...
                          const crossed =
                            policy.monthlyBudgetUSD === undefined
                              ...
                              : thresholds.filter(
                                  (threshold) =>
                                    spend / policy.monthlyBudgetUSD! >= threshold &&
                                    !sent.includes(threshold),
                                );
                          await req.frogbot.update({
                            collection: target.collection as never,
                            id: target.id as never,
                            data: {
                              spendThisPeriodUSD: spend,
                              budgetPeriodStartedAt:
                                doc.budgetPeriodStartedAt ?? new Date().toISOString(),
                              budgetAlertsSent: [...sent, ...crossed],
                            },
                            overrideAccess: true,
                            req,
                          });
