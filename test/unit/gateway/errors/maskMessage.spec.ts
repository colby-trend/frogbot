...
describe('maybeMaskMessage', () => {
  it('passes through in non-production', () => {
    expect(maybeMaskMessage('boom', { status: 500, production: false })).toBe('boom');
  });

  it('passes through 4xx in production (client-actionable)', () => {
    expect(maybeMaskMessage('bad key', { status: 401, production: true })).toBe('bad key');
  });

  it('masks 5xx in production', () => {
    expect(maybeMaskMessage('internal boom', { status: 500, production: true })).toBe(
...
describe('redactKeyFragments (G34)', () => {
  ...
  it('redacts the masked-fragment shape OpenAI actually emits', () => {
    expect(redactKeyFragments('Incorrect API key provided: sk-proj-********abc1.')).toBe(
      'Incorrect API key provided: [REDACTED_KEY].',
    );
  });
