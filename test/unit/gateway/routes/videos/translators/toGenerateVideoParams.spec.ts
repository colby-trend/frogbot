...
import { toGenerateVideoParams } from '../../../../../../packages/gateway/src/routes/videos/translators/toGenerateVideoParams.js';
...
describe('toGenerateVideoParams', () => {
  ...
  it.each([
    ...
    'vertex',
    'xai',
  ])('scopes video provider options for %s', (providerName) => {
    expect(
      toGenerateVideoParams({
        body: {
  ...
  it('maps timeout_ms to pollTimeoutMs', () => {
    ...
    ).toEqual({ replicate: { pollTimeoutMs: 300000 } });
  });
});
