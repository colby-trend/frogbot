...
import { oauthPlugin } from '../../../../packages/plugins/plugin-oauth/src/index.js';
...
async function setup(overrides: Partial<OAuthProvider> = {}) {
  const provider: OAuthProvider = {
    ...
    scopes: [],
    authorize: () => new URL('https://provider.test/auth'),
...
function request({
  ...
  return {
    req: {
      ...
      user: { id: owner },
      json: () => Promise.resolve({ connectionId: 'connection-1' }),
      frogbot: {
...
describe('OAuth connection lifecycle', () => {
  it('refreshes tokens while retaining an omitted refresh token', async () => {
    ...
    const encryptedCredentials = await encryption.encrypt(
      JSON.stringify({ access_token: 'access', refresh_token: 'keep' }),
    );
    const { req, update } = request({ encryptedCredentials });
    expect((await refresh.handler(req)).status).toBe(200);
    const stored = JSON.parse(
      await encryption.decrypt(update.mock.calls[0][0].data.encryptedCredentials),
    );
  ...
  it('revokes locally when provider revocation fails and denies cross-owner lookup', async () => {
    ...
    expect(update).toHaveBeenCalledWith(
      expect.objectContaining({ data: { status: 'revoked', encryptedCredentials: '' } }),
    );
