...
describe('OAuth plugin integration', () => {
  it('composes authorize, persistence, resolution, piece execution, refresh, and revoke', async () => {
    ...
    const connections = transformed.collections.find(
      (collection) => collection.slug === 'connections',
    )!;
    ...
    const callback = accounts.endpoints!.find(
    ...
    const callbackResponse = await callback.handler({
      ...
      frogbot: {
        delete: vi.fn().mockResolvedValue({ docs: [{ id: 'state-1', ...state }] }),
        find,
        create,
      },
    } as unknown as FrogbotRequest);
    ...
    await expect(
      piece.tools()[0]!.execute({}, {
        req: { user: { id: 'user-1' } },
        frogbot: { connections: connectionAPI },
      } as never),
    ).resolves.toMatchObject({ type: 'OAUTH2', access_token: 'access', refresh_token: 'refresh' });
