  ...
  if (url.includes('bedrock-mantle')) {
    if (url.endsWith('/responses')) {
      return new Response(JSON.stringify({
        ...
        output: [{ id: 'message-1', type: 'message', role: 'assistant', status: 'completed', content: [{ type: 'output_text', text: 'ok', annotations: [] }] }],
        status: 'completed',
...
describe('Bedrock file content wire contract', () => {
  ...
  it('sends messages inline image bytes as base64', async () => {
    await postJson(makeApp(), '/v1/messages', {
      model: `amazon-bedrock/${STANDARD_MODEL}`,
      max_tokens: 10,
      messages: [{ role: 'user', content: [{ type: 'image', source: { type: 'base64', media_type: 'image/png', data: PNG } }] }],
    });
  ...
  it('sends tool-result image bytes without throwing', async () => {
    const { status } = await postJson(makeApp(), '/v1/messages', {
      model: `amazon-bedrock/${STANDARD_MODEL}`,
      max_tokens: 10,
      tools: [{ name: 'look', description: 'Look', input_schema: { type: 'object', properties: {} } }],
      messages: [
        { role: 'user', content: 'Look' },
        { role: 'assistant', content: [{ type: 'tool_use', id: 'tool-1', name: 'look', input: {} }] },
        { role: 'user', content: [{ type: 'tool_result', tool_use_id: 'tool-1', content: [{ type: 'image', source: { type: 'base64', media_type: 'image/png', data: PNG } }] }] },
      ],
    });

    expect(status).toBe(200);
    expect((upstreamBodies[0] as any).messages[2].content[0].toolResult.content[0].image.source.bytes).toBe(PNG);
  });
  ...
  it('sends Mantle chat image data as a valid data URL', async () => {
    ...
    vi.spyOn(DEFAULT_MODEL_CATALOG, 'get').mockImplementation((id) => id === key
      ? {
          ...get(id)!,
          sdk: {
    ...
    expect((upstreamBodies[0] as any).messages[0].content[0].image_url.url)
  ...
  it('sends Mantle responses image data as a valid data URL', async () => {
    ...
    expect((upstreamBodies[0] as any).input[0].content[0].image_url)
      .toBe(`data:image/png;base64,${PNG}`);
  });
