...
All `frogbot` and `@frogbotai/*` packages must be on exactly the same version and installed only once.
...
`@frogbotai/ui` intentionally contains two bundles of itself, so you may see dual paths even when everything is correct. Inside the FrogBot Admin UI you must import only:

- `@frogbotai/ui`
- `@frogbotai/ui/rsc`
- `@frogbotai/ui/shared`

Any other deep import such as `@frogbotai/ui/elements/Button` should **only** be used in your own frontend, outside of the FrogBot Admin Panel. Those deep entries are published un-bundled to help you tree-shake and ship a smaller client bundle if you only need a few components from `@frogbotai/ui`.
...
- Run [Syncpack](https://www.npmjs.com/package/syncpack) to enforce identical versions of every `@frogbotai/*`, `react`, and `react-dom` reference.
...
This is a common pitfall when using a monorepo setup with multiple packages. In this case, ensure that all packages in the monorepo use the same version of `frogbot`, `@frogbotai/*`, `next`, `react`, and `react-dom`. You can use pnpm with workspaces to manage dependencies across packages in a monorepo effectively. Unfortunately this error becomes harder to debug inside a monorepo due to how package managers hoist dependencies as well as resolve them.
