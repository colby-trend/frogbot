...
Collections you configure remain ordinary Payload collections. FrogBot injects a files collection, and agents also need thread and message stores. You can adopt those roles into collections you own.
...
The following options are available on any `CollectionConfig`. FrogBot adds `thread`, `message`, `connections`, and `file` to Payload's standard set; everything else below is Payload's native API.
