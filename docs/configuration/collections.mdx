...
Collections you configure remain ordinary FrogBot collections. FrogBot injects a files collection, and agents also need thread and message stores. You can adopt those roles into collections you own.

### Role markers

FrogBot identifies managed collections by role, not by a required slug. Mark a collection with `thread: true`, `message: true`, or `file: true`; FrogBot keeps its slug and merges in the required configuration.
...
The following options are available on any `CollectionConfig`. FrogBot adds `thread`, `message`, `connections`, and `file` to the built-in set; every other option below is part of the FrogBot configuration API.
